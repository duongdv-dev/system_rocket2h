...
class FeatureExtractor:
    """
    Feature Extractor cho Master System 13 Lớp:
    Trích xuất đầy đủ 12+ chỉ số đặc trưng trước 10:00 AM ICT để phục vụ 13 Bước AI.
    """
    ...
    def extract_daily_features(self):
        ...
        for date_str in trading_days:
            ...
            # Tính toán dữ liệu 06:00 - 10:00
            t_0am = time(0, 0, 0)
            t_6am = time(6, 0, 0)
            t_8am = time(8, 0, 0)
            t_9h30 = time(9, 30, 0)

            daily_start = day_m1[day_m1['time'] >= t_0am]
            daily_open_price = float(daily_start.iloc[0]['open']) if not daily_start.empty else anchor_open
            open_daily_dist = anchor_open - daily_open_price
            ...
            if not morning_df.empty:
                ...
                morning_direction = 1 if (anchor_open >= morning_open) else -1

                # Bien do 30m / 60m truoc 10h
                df_60m = day_m1[(day_m1['time'] >= t_9h30) & (day_m1['time'] <= target_10am)]
                range_30m = float(df_60m['high'].max() - df_60m['low'].min()) if not df_60m.empty else morning_range_pts * 0.5
            ...
            feature_records.append({
                ...
                "open_daily_dist": round(open_daily_dist, 3),
                "range_30m": round(range_30m, 3),
