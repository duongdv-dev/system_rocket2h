...
def run_master_system_comparison():
    ...
    for log in master_logs:
        ...
        log['status'] = "SKIPPED" if log['active_lot_size'] == 0.00 else ("ATTACK" if log['step_multiplier'] < 0.60 else "DEFENSE")
    ...
    comparison_report = {
        ...
        "test_phase_2023_2024": {
            ...
            "master_system_combined": {
                ...
                "attack_days_count": len(df_master[df_master['step_multiplier'] < 0.60]),
                "defense_days_count": len(df_master[(df_master['active_lot_size'] > 0.00) & (df_master['step_multiplier'] >= 0.60)]),
    ...
    print(f"Attack Days (0.50 Step)  | 0 days                   | {len(df_master[df_master['step_multiplier'] < 0.60])} days")
    print(f"Defense Days (0.65 Step) | 0 days                   | {len(df_master[(df_master['active_lot_size'] > 0.00) & (df_master['step_multiplier'] >= 0.60)])} days")
