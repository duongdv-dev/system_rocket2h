...
class StepBacktester(DCABacktester):
    def __init__(self, data_paths, ai_model_path=None, initial_balance=10000.0, default_lot=0.10, max_daily_loss_pct=5.0):
        ...
        self.step_optimizer = StepOptimizer(base_lot=default_lot, min_lot=0.02, safe_step_mult=0.50, moderate_step_mult=0.85)
    ...
    def calculate_master_daily_configs(self, features_df):
        """
        Calculates daily (lot_size, step_mult) for each day based on AI predicted P(Risk).
        Returns a dict: {date_str: {'lot': float, 'step_mult': float, 'reason': str}}
        """
        config_map = {}
